...
/**
 * @brief Reprezentuje graf složený z jednotlivých vrcholů spojených hranami
 * 
...
public:
     ...
     * 
     * Metoda n-krát, kde n je vertexCount, volá na vektor vertices metodu emplace_back(uint32_t), 
     ...
     * konkrétní objekt třídy Vertex a zavolá jeho metodu addNeighbor(uint32_t), která do vnitřního vektoru (std::vector<uint32_t> neighbors) daného objektu přidá index předaný druhým parametrem.
     ...
     * 
     * Principem ověření bipartitnosti je rozdělení vrcholů do dvou množin s tím, že dva sousední vrcholy nesmí ležet ve stejné množině. Vrcholy není potřeba doslovně přidávat do množin, ale stačí je nějak označit.
     ...
     * 
     * Konkrétních metod pro ověření bipartitnosti grafu je několik. Mezi ty základní patří průchod do hloubky (DFS) a průchod do šířky (BFS). Oba způsoby mohou navíc být realizovány jak iterativně, tak rekurzivně.
     * Tato konkrétní metoda využívá průchodu do hloubky. V případě, kdy by vyhledávání bylo realizováno pomocí rekurze, tak by při obrovském množství vrcholů mohlo eventuálně dojít k nedostatku místa na zásobníku.
     ...
     * 
     * Jestliže se jedná o vrchol, který už má přidělenou barvu (vertex.getColor() != NONE), lze s jistotou říct, že i všechny jeho sousední vrcholy jsou již označeny barvou a nedochází u nich ke kolizi, proto je možné následující kroky přeskočit.
     * 
     * Pokud vrchol ještě není označen barvou, je mu přiřazena výchozí barva (GREEN) a jeho index je přidán do zásobníku.
     * 
     * Následuje cyklus s podmínkou na konci, který běží, dokud zásobník není prázdný.
     ...
     * Pokud sousední vrchol barvu má a shoduje se s barvou aktuálně zkoumaného vrcholu (neighborColor == currentColor), můžeme ihned vrátit hodnotu false, protože se jedná o porušení základního pravidla bipartitního grafu.
     * Pokud sousední vrchol barvu má a neshoduje se s barvou aktuálně zkoumaného vrcholu, pravidlo bipartitnosti je splněno a můžeme přejít na zkoumání dalšího ze sousedních vrcholů.
     * Pokud sousední vrchol barvu ještě nemá, je mu přiřazena jiná barva než má vrchol, jehož sousedi jsou zkoumáni. Index tohoto sousedního vrcholu je pak umístěn na vrchol zásobníku a jelikož ukončovací podmínka cyklu s podmínkou na konci (!stack.empty()) není splněna, pro jeho sousedy je celý proces vykonán znovu.
     * 
